...
public GameObject WhoOwns; //храним обьект, которому принадлежит оружие (например игрока или врага)
     ...
     void Update()
    {
       ...
       public void OnTriggerEnter(Collider other) {
        ...
        if ((other.gameObject != WhoOwns && other.gameObject.tag != "Weapon") && other.gameObject.tag == "LeftHand" || other.gameObject.tag == "RightHand" || other.gameObject.tag == "LeftLeg" || other.gameObject.tag == "RightLeg" || other.gameObject.tag == "Torso" || other.gameObject.tag == "Head") { //если мы попали не по своему основному коллайдеру и тег по которому попали не равняется Weapon
...
other.gameObject.GetComponent<Organ>().parent.GetComponent<GetHit>().GetHitVoid(100); // проигрываем анимацию получения урона
       ...
       if (this.enabled == true) {
other.gameObject.GetComponent<Organ>().parent.GetComponent<GetHit>().GetHitVoid(20); // проигрываем анимацию получения урона
